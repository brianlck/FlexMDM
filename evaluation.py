...
def model_load(model_name: str):
    if model_name == "vlmdm":
        checkpoint_path = "/n/netscratch/sham_lab/Everyone/jay_brian/our_model/epoch-epoch=505-val_loss-val_loss=1.8743.ckpt"
        model = TransdimensionalFlowModule.load_from_checkpoint(checkpoint_path)
    elif model_name == "mdm":
        checkpoint_path = "/n/netscratch/sham_lab/Everyone/jay_brian/mdm/epoch-epoch=73-val_loss-val_loss=1.7349.ckpt"
...
def eval_gen_ppl(text_samples, max_length: int = 512, retokenize: bool = True) -> None:
    ...
    eval_model_tokenizer = AutoTokenizer.from_pretrained(
        "meta-llama/Llama-2-7b-hf", cache_dir=custom_cache_dir
    )
...
def calculate_entropy(tokenized_tensor_batch, first_eos_batch):
    ...
    for tokenized_tensor, first_eos in zip(tokenized_tensor_batch, first_eos_batch):
        ...
        if first_eos.any():
            first_eos_idx = torch.where(first_eos)[0][
                0
            ].item()  # Get the first index where EOS is True
            valid_tokens = tokenized_tensor[
                :first_eos_idx
            ]  # Slice tokens up to the first EOS
        else:
...
def calculate_mauve_score(
    ...
    max_length: int,
    reference_dataset: str = "wikitext2",
    num_reference: int = 2,
) -> float:
    # load true (reference) dataset
    dataset = get_text_dataset(
        name=reference_dataset, split="train", max_length=max_length, num_proc=8
    )

    # convert token IDs back to strings
    tokeniser = setup_tokeniser_from_dataset(reference_dataset)
    ...
    refer_texts = decode_sequence_with_mask(
        refer_tokens, tokeniser, pad_token, mask_token
    )
...
def length_stat_plot(metrics, model_name="mdm", data="wikitext2"):
    ...
    plt.title("Length distribution of generated samples")
...
def main(args):
    ...
    tokeniser = setup_tokeniser(args.model_tokeniser)
    processing_size = 16
    ...
    for b_idx in range(num_batches):
        ...
        if args.model == "vlmdm":
            sampling_trace, len_trace = mdm_style_sampling(
                model,
            ...
            sampling_trace, len_trace = mdm_sampling(
                model,
                num_steps=args.num_steps,
                tokeniser=tokeniser,
                mask=tokeniser.mask_token_id,
    ...
    if args.genppl:
        print(
            f"Average likelihood: {sum([m['likelihood'] for m in metrics]) / len(metrics)}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt2",
        help="Tokeniser used for decoding model samples, choices are ['gpt2', 'gpt-neo']",
    )
    # sampler parameters
    parser.add_argument("--num_steps", type=int, default=1024)
    parser.add_argument(
        "--num_samples", type=int, default=1024, help="need to be the multiple of 16"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="temperature for categorical sampling",
    )
